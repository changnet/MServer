...
function Res:add_one_res(player, res, op, sub_op)
    ...
    if not add_func then
        ERROR("Res:add_one_res no function found:%s,op %d", tostring(res.res),
              op)
        return
    end
...
function Res:dec_one_res(player, res, op, sub_op)
    ...
    if not dec_func then
        ERROR("Res:dec_one_res no function found:%s,op %d", tostring(res.res),
              op)
        return
    end
...
function Res:reg_player_res(res_type, get, add, dec)
    ...
    res_add_map[res_type] = function(player, ...)
        return add(player, ...)
    end
    res_dec_map[res_type] = function(player, ...)
        return dec(player, ...)
    end
end
