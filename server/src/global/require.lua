...
-- 根据当前worker类型判断是否加载文件
-- @param wtype worker类型，不传表示所有worker都加载，~wtype表示该类型worker不加载
function require_worker(path, wtype, ...)
    if not wtype then return require(path) end
