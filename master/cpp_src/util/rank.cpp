...
{
    for (int32 idx = object->_index + 1;idx < _rank_count;idx ++)
    {
        object_t *next = _object_list[idx];
        if (object_t::compare_factor(
            object->_factor,next->_factor,_max_factor) >= 0)
...
{
    _rank_count --;
    for (int32 idx = object->_index;idx < _rank_count;idx ++)
    {
...
int32 insertion_rank::insert(object_id_t id,factor_t factor,int32 max_idx)
{
    ...
    // 排行榜已满处理
    if (_rank_count >= _max_count)
    {
        object_t *last = _object_list[_rank_count - 1];
    ...
    array_resize(object_t*,_object_list,_rank_count,_max_count,array_zero);

    object->_index = _rank_count;
    _object_list[_rank_count++] = object;
    ...
    object_id_t id,raw_factor_t factor,int32 factor_idx)
{
    ...
    raw_factor_t old_factor = object->_factor[factor_idx];

    // 在实际应用中，很少会出现同时更新几个排序因子的情况，因此只给单个因子更新接口
    if (old_factor == factor) return 0;
    object->_factor[factor_idx] = factor;
...
{
    if (rank < 0 || rank >= _rank_count) return -1;

    return _object_list[rank]->_id;
}
