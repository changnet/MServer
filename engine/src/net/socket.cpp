...
Socket::Socket(int32_t socket_id)
{
    ...
    af_type_ = (AF_INET << 16) | (SOCK_STREAM << 8);
    io_type_    = IO::IOT_TCP;
}
...
int32_t Socket::set_keep_alive(int32_t time, int32_t interval, int32_t probes)
{
    ...
    return setsockopt(fd, SOL_TCP, TCP_KEEPCNT, &tcp_keepalive_probes, optlen);
#endif
}

int32_t Socket::set_nodelay(int32_t opt)
{
    /**
...
#ifdef __windows__
    DWORD optval = opt;
    return setsockopt(w_->fd_, IPPROTO_TCP, TCP_NODELAY, (char *)&optval,
                      sizeof(optval));
#else
    int optval = opt;
    return setsockopt(w_->fd_, IPPROTO_TCP, TCP_NODELAY, (void *)&optval,
                      sizeof(optval));
#endif
}

int32_t Socket::set_user_timeout(int32_t timeout)
{
...
int32_t Socket::connect(int32_t addr, const char *host, int32_t port)
{
    ...
    if (!w_->io_)
    {
        ELOG("socket connect io is null");
        return -1;
    }

    int32_t af = af_type_ >> 16;
    int32_t type = (af_type_ >> 8) & 0xFF;

    // 创建新socket并设置为非阻塞
    int32_t fd = netcompat::INVALID;
    if (type == SOCK_STREAM)
    {
        fd = (int32_t)::socket(af, SOCK_STREAM, IPPROTO_IP);
    }
    else if (type == SOCK_DGRAM)
    {
        fd = (int32_t)::socket(af, SOCK_DGRAM, IPPROTO_UDP);
    }
