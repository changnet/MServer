...
private:
    std::vector<Timer *> timers_; /// 按二叉树排列的定时器
    std::unordered_map<int32_t, Timer> timer_mgr_;

    std::vector<Timer *> periodics_; /// 按二叉树排列的utc定时器
    std::unordered_map<int32_t, Timer> periodic_mgr_;
};
